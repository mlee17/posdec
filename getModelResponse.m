...
%
%        $Id:$ 
%      usage: getModelResponse(v,roi,iFold)
%         by: minyoung lee
%       date: 01/25/17
%    purpose: 
%
function [roi] = getModelResponse(v, roi, iFold)
...
% roin=0;
disppercent(-inf,'(getModelResponse) estimating beta weights');
for iVox = 1:nVox
...
%     rfModel = exp(-(((pRFd.stimX-rfX).^2)/(2*(rfHalfWidth^2))+((pRFd.stimY-rfY).^2)/(2*(rfHalfWidth^2))));
    % init model response
    thisModelResponse = [];
    for cond = 1:length(stim.xpos)*length(stim.contrast)
%     for locnum = 1:length(stim.xpos)
%         for connum = 1:length(stim.contrast)
%             condition = length(stim.contrast)*(locnum-1)+connum;
            stimX = stim.xpos(ceil(cond/2));
%             if ~any(isnan(roi.pRF.params(:,voxnum)))
            thisModelResponse = convolveModelWithStimulus(x,y,sigma,stimX,stimY,stimSigma);
            modelResponse{cond}(:,iVox) = repmat(thisModelResponse,stim.length/framePeriod,1);
            
            modelRespConvolved = convolveModelResponseWithHRF(modelResponse{cond}(:,iVox), hrf);

function [fit, fieldNames] = pdFitCG(sbjID)
	...
	for i = 1:length(fieldNames)
		...
		pfitVal = arrayfun(@(x) cgfunc(mu,sigma,lambda,B,x), testVal);
    	...
    	plot(xs, fitVal, 'k')
    	if taskType==1
    		title(sprintf('Stair Contrast: Ecc=%d   (N=%d) \n \\mu=%0.2f \\sigma=%0.2f \\lambda=%0.2f B=%0.2f \n', ...
    			stimStr(1).eccentricity, nTrialCombined, mu,sigma,lambda,B), 'Interpreter','tex');
    		ylabel('Percent Correct');
    		xlabel('Contrast');
    		axis([0 0.6 0 1]); box off; drawnow;
    	else
    		title(sprintf('Ecc=%d   Con=%0.3f   (N=%d) \n \\mu=%0.2f \\sigma=%0.2f \\lambda=%0.2f B=%0.2f \n', ...
    			stimStr(1).eccentricity, stimStr(1).stimulus.contrast, nTrialCombined, mu,sigma,lambda,B), 'Interpreter','tex');

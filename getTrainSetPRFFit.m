...
%    purpose: 
%
function [roi,m] = getSortPRFFit(v, roi)
...
%             if ~any(isnan(roi.pRF.params(:,voxnum)))
            ...
            modelResponse{h}{locnum}{connum}(:,iVox) = repmat(thisModelResponse,stim.length/framePeriod,1);
            
            modelRespConvolved = convolveModelResponseWithHRF(modelResponse{h}{locnum}{connum}(:,iVox), hrf);
...
%             end
            ...
            [modelRespConvolved, residual{h}{locnum}{connum}(iVox,:), beta{h}{locnum}{connum}(:,iVox)] ...
